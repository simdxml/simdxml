...
fn apply_predicate<'a>(
    ...
    // Fast path: [@attr='literal'] or [@attr="literal"] — SIMD memmem scan
...
///
/// Instead of calling get_attribute() per node (O(candidates × tag_bytes)),
/// scans the entire input for the byte pattern `attr="value"` or `attr='value'`
